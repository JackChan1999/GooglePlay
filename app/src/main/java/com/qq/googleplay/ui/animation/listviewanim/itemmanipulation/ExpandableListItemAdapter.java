...
/**
 * An {@link com.qq.googleplay.ui.animation.listviewanim.ArrayAdapter} which allows items to be expanded using an animation.
 */
public abstract class ExpandableListItemAdapter<T> extends ArrayAdapter<T> {
	...
	private static class RootView extends LinearLayout {
		...
		private void init() {
			...
			mTitleViewGroup.setId(R.id.titleViewGroup);
			addView(mTitleViewGroup);

			mContentViewGroup = new FrameLayout(getContext());
			mContentViewGroup.setId(R.id.contentViewGroup);
